...
{
    version (GNU) {
	return gcc.config.frexpl(x, & exponent);
    } else {
	asm
	{
		fld x;
...
// BUG: this code is broken, returning a pointer into the stack
	...
	while (true)
	{
		...
		{
		    // %% Lg on dawrin?
		    count = std.c.stdio.snprintf(p, psize, "%Lg", x);
		    if (count == -1)
			    psize *= 2;
		    else if (count >= psize)
			    psize = count + 1;
