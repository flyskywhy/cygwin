...
struct Box
{
	...
	version (GNU)
	{
	    // This branch could be used on all targets, but using the above
	    // will help work out bugs for the baseline distro.
	    TypeInfo ti = type;
	    while (1) {
		switch (ti.classinfo.name[9]) {
		case 'b': return .toString(*cast(bit*)data);
		case 'a': return .toString(*cast(char*)data);
		case 'u':
		    {
			char[] string;
			encode(string, *cast(wchar*)data);
			return string;
		    }
		case 'w':
		    {
			char[] string;
			encode(string, *cast(dchar*)data);
			return string;
		    }
		case 'g': return .toString(*cast(byte*)data);
		case 'h': return .toString(*cast(ubyte*)data);
		case 's': return .toString(*cast(short*)data);
		case 't': return .toString(*cast(ushort*)data);
		case 'i': return .toString(*cast(int*)data);
		case 'k': return .toString(*cast(uint*)data);
		case 'l': return .toString(*cast(long*)data);
		case 'm': return .toString(*cast(ulong*)data);
		case 'f': return .toString(*cast(float*)data);
		case 'd': return .toString(*cast(double*)data);
		case 'e': return .toString(*cast(real*)data);
		case 'o': return .toString(*cast(ifloat*)data);
		case 'p': return .toString(*cast(idouble*)data);
		case 'j': return .toString(*cast(ireal*)data);
		case 'q': return .toString(*cast(cfloat*)data);
		case 'r': return .toString(*cast(cdouble*)data);
		case 'c': return .toString(*cast(creal*)data);
		case 'P': return .format("%s",*cast(void**)data);
		case 'C': return (*cast(Object*)data).toString();
		case 'T': ti = (cast(TypeInfo_Typedef)ti).base;
		default:
		    // doFormat raises errors for the rest:
	    ...
	    version (GNU)
		std.format.doFormat(&putc, arguments, cast(va_list) args);
	    else
...
body
{
	...
	version (BigEndian)
	{
	    byte b;
	    short s;
	    switch (_arguments[0].tsize()) {
	    case 1: b = va_arg!(byte)(_argptr); p = & b; break;
	    case 2: s = va_arg!(short)(_argptr); p = & s; break;
	    default: p = cast(void *) _argptr;
	    }
	}
	else
	    p = cast(void *) _argptr;
	return box(_arguments[0], p);
    }
    else
