{ This file was generated automatically from gpc-pas.in.
...
function  UMask (Mask: Integer): Integer; attribute (ignorable); external name '_p_UMask';
...
procedure GetBinding   (protected var aFile: AnyFile; var aBinding: BindingType); external name '_p_GetBinding';
...
procedure CloseFile (var aFile: AnyFile); external name '_p_CloseFile';
...
procedure RTSInitFile; external name '_p_init_GPC_File';

{ Mathematical routines, from math.pas }

function Ln1Plus  (x: Real): Real; attribute (const, name = '_p_Ln1Plus'); external;
...
function  MemCmp      (const s1, s2; Size: SizeType): Integer; external name 'memcmp';
function  MemComp     (const s1, s2; Size: SizeType): Integer; external name 'memcmp';
...
function  UpCaseStr       (const s: String): TString;                             attribute (name = '_p_UpCaseStr'); external;
function  LoCaseStr       (const s: String): TString;                             attribute (name = '_p_LoCaseStr'); external;
...
function  NewString       (const s: String): PString;                            attribute (name = '_p_NewString'); external;
...
function  TrimLeftStr     (const s: String): TString;                            attribute (name = '_p_TrimLeftStr'); external;
function  TrimRightStr    (const s: String): TString;                            attribute (name = '_p_TrimRightStr'); external;
function  TrimBothStr     (const s: String): TString;                            attribute (name = '_p_TrimBothStr'); external;
function  LTrim           (const s: String): TString;                            external name '_p_TrimLeftStr';
...
function  Integer2String (i: Integer): Str64;                                    attribute (name = '_p_Integer2String'); external;
...
procedure DebugStatement                  (const FileName: String; Line: Integer); attribute (name = '_p_DebugStatement'); external;
...
procedure IOErrorFile                     (n: Integer; protected var f: AnyFile; ErrNoFlag: Boolean); attribute (iocritical, name = '_p_IOErrorFile'); external;

function  GetIOErrorMessage: TString;                                              attribute (name = '_p_GetIOErrorMessage'); external;
...
function  ReturnAddr2Hex (p: Pointer): TString; attribute (name = '_p_ReturnAddr2Hex'); external;
...
function  StrSignal (Signal: Integer): TString; attribute (name = '_p_StrSignal'); external;
...
function  FormatTime (const Time: TimeStamp; const Format: String): TString; attribute (name = '_p_FormatTime'); external;
...
function  Slash2OSDirSeparator (const s: String): TString;               attribute (name = '_p_Slash2OSDirSeparator'); external;

{ Change a file name to use '/' as directory separator }
function  OSDirSeparator2Slash (const s: String): TString;               attribute (name = '_p_OSDirSeparator2Slash'); external;

{ Like Slash2OSDirSeparator for CStrings. *Note*: overwrites the
  CString }
function  Slash2OSDirSeparator_CString (s: CString): CString;            attribute (name = '_p_Slash2OSDirSeparator_CString'); external;

{ Like OSDirSeparator2Slash for CStrings. *Note*: overwrites the
  CString }
function  OSDirSeparator2Slash_CString (s: CString): CString;            attribute (name = '_p_OSDirSeparator2Slash_CString'); external;

{ Add a DirSeparator to the end of s, if there is not already one
  and s denotes an existing directory }
function  AddDirSeparator (const s: String): TString;                    attribute (name = '_p_AddDirSeparator'); external;

{ Like AddDirSeparator, but also if the directory does not exist }
function  ForceAddDirSeparator (const s: String): TString;               attribute (name = '_p_ForceAddDirSeparator'); external;
...
function  RemoveDirSeparator (const s: String): TString;                 attribute (name = '_p_RemoveDirSeparator'); external;
...
{ Returns a non-existing file name in the directory given. If the
  ...
  string. }
function  GetTempFileNameInDirectory (const Directory: String): TString; attribute (iocritical, name = '_p_GetTempFileNameInDirectory'); external;
...
function  FileName (protected var f: AnyFile): TString;                  attribute (name = '_p_FileName'); external;
...
function  FSearchExecutable (const aFileName, DirList: String): TString; attribute (name = '_p_FSearchExecutable'); external;
...
function  RelativePath (const Path: String; OnlyCurDir, Quoted: Boolean): TString; attribute (name = '_p_RelativePath'); external;
...
{ Functions that extract one or two of the parts from FSplit.
  ...
  directory. }
function  DirFromPath     (const Path: String): TString;                 attribute (name = '_p_DirFromPath'); external;
function  NameFromPath    (const Path: String): TString;                 attribute (name = '_p_NameFromPath'); external;
function  ExtFromPath     (const Path: String): TString;                 attribute (name = '_p_ExtFromPath'); external;
...
function  OpenDir  (const DirName: String): DirPtr;                      attribute (iocritical, name = '_p_OpenDir'); external;
...
function  QuoteFileName (const s: String; const SpecialCharacters: CharSet): TString; attribute (name = '_p_QuoteFileName'); external;

{ Remove QuotingCharacter from s }
function  UnQuoteFileName (const s: String): TString;                    attribute (name = '_p_UnQuoteFileName'); external;
...
function  BraceExpand (const s: String): PPStrings;                      attribute (name = '_p_BraceExpand'); external;
...
{ Finds a password entry by user name. Returns True if found, False
  otherwise. }
function  GetPasswordEntryByName (const UserName: String; var Entry: TPasswordEntry): Boolean; attribute (name = '_p_GetPasswordEntryByName'); external;

{ Finds a password entry by UID. Returns True if found, False
  otherwise. }
function  GetPasswordEntryByUID  (UID: Integer; var Entry: TPasswordEntry): Boolean; attribute (name = '_p_GetPasswordEntryByUID'); external;

{ Returns all password entries, or nil if none found. }
function  GetPasswordEntries: PPasswordEntries; attribute (name = '_p_GetPasswordEntries'); external;
...
function  GetMountPoint (const Path: String): TString; attribute (name = '_p_GetMountPoint'); external;
...
{ Returns system information if available. Fields not available will
  be empty. }
function  SystemInfo: TSystemInfo; attribute (name = '_p_SystemInfo'); external;
...
{ Returns the path to the shell (as the result) and the option that
  ...
  interactively). }
function  GetShellPath (var Option: String): TString; attribute (name = '_p_GetShellPath'); external;
...
const
  IOSelectEventMin = Low (IOSelectEvents);
...
{ Under development }
procedure AnyStringTFDD_Reset (var f: AnyFile; var Buf: ConstAnyString); attribute (name = '_p_AnyStringTFDD_Reset'); external;
{ @@ procedure AnyStringTFDD_Rewrite (var f: AnyFile; var Buf: VarAnyString); attribute (name = '_p_AnyStringTFDD_Rewrite'); }
procedure StringTFDD_Reset (var f: AnyFile; var Buf: ConstAnyString; const s: String); attribute (name = '_p_StringTFDD_Reset'); external;
...
function  IsTerminal (protected var f: AnyFile): Boolean; attribute (name = '_p_IsTerminal'); external;
...
function  GetTerminalName (protected var f: AnyFile): TString; attribute (name = '_p_GetTerminalName'); external;
