...
function TokenizeString (const Source: String; Separators: CharSet): PPStrings;
...
function ShellTokenizeString (const s: String; var ErrMsg: String): PPStrings;
...
{ Check that a certain string is contained in s (after possible
  space characters). }
function  StrReadConst (const s: String; var i: Integer; const Expected: String): Boolean; attribute (ignorable);

{ A simpler to use version of StrReadConst that expects a `,'. }
function  StrReadComma (const s: String; var i: Integer): Boolean; attribute (ignorable);
...
begin
  ...
  s1 := s[i .. j];
  i := j + 1;
  if not UnQuoteString (s1) or (Length (s1) > GetStringCapacity (Dest)) then Exit;
  Dest := s1;
  StrReadQuoted := True
end;
...
begin
  ...
  if (j = 0) or (j - i - 1 > GetStringCapacity (Dest)) then Exit;
...
begin
  ...
  if j - i > GetStringCapacity (Dest) then Exit;
  Dest := Copy (s, i, j - i);
  i := j;
  StrReadWord := True
end;
